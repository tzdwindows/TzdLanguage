...
#### `--noJit` 或 `--no-jit`
强制关闭 JIT 实时编译优化，所有代码均在最新深度优化的堆栈式字节码虚拟机（Bytecode VM, Tier 0）中执行。
- **极致优化体系**：搭载扁平化迭代调用调度（消除 C++ 递归调用）、`INC_LOCAL` 本地变量自增特化指令、标量极速传输（避开 380 字节 STL 容器拷贝）、调用点静态索引缓存与零拷贝原地返回值；
- **极限吞吐突破**：实测 100 万次函数调用仅需 **0.089 秒**（相比旧版解释器提速 **83.5 倍**，单次调用仅 ~89ns）；
- **适用场景**：
  - 内存受限或无原生编译环境的设备；
  - 极速冷启动，无需等待 JIT 后端编译；
  - 性能要求严苛但需排查 JIT 编译行为差异时的对照基准。
