; ==============================================================================
; TzdTools Windows Installer (NSIS Script)
...
OutFile "..\dist\TzdTools_Setup_v0.2.4.exe"
...
Section "MainSection" SEC01
    ...
    ; Extract complete TzdTools runtime & dependencies into $INSTDIR
    DetailPrint "Extracting TzdTools runtime and CUDA/LibTorch dependencies (3.7 GB)..."
    nsExec::ExecToLog '"$PLUGINSDIR\7z.exe" x "$PLUGINSDIR\payload.7z" -o"$INSTDIR" -y'
    Pop $0
    ${If} $0 != 0
    ...
    WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "${PRODUCT_NAME} (TzdLang Runtime & Compiler)"
    ...
    WriteRegDWORD ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "EstimatedSize" 3820000
