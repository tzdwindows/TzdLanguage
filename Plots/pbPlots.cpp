...
vector<RGBA*>* Get8HighContrastColors() {
    ...
    colors->at(0) = CreateRGBColor(0.12, 0.47, 0.71); // 深蓝
    colors->at(1) = CreateRGBColor(1.00, 0.50, 0.05); // 橙色
    colors->at(2) = CreateRGBColor(0.17, 0.63, 0.17); // 绿色
    colors->at(3) = CreateRGBColor(0.84, 0.15, 0.16); // 红色
    colors->at(4) = CreateRGBColor(0.58, 0.40, 0.74); // 紫色
    colors->at(5) = CreateRGBColor(0.55, 0.34, 0.29); // 棕色
    colors->at(6) = CreateRGBColor(0.89, 0.47, 0.76); // 粉色
    colors->at(7) = CreateRGBColor(0.50, 0.50, 0.50); // 灰色
    return colors;
}
...
bool DrawScatterPlotFromSettings(RGBABitmapImageReference* canvasReference, ScatterPlotSettings* settings, StringReference* errorMessage) {
    ...
    if (success) {
        ...
        double xValuePerPixel = (xMax - xMin) / (xPixelMax - xPixelMin);
        double yValuePerPixel = (yMax - yMin) / (yPixelMax - yPixelMin);

        // 根据 Padding 大小，推算出图片边缘对应的数值
        double extendedXMin = xMin - (xPixelMin * xValuePerPixel);
        double extendedXMax = xMax + ((settings->width - xPixelMax) * xValuePerPixel);
        double extendedYMin = yMin - (yPadding * yValuePerPixel);
        double extendedYMax = yMax + ((settings->height - yPixelMax) * yValuePerPixel);

        // 使用扩展后的边界来生成网格位置
        // 这样计算出来的 xGridPositions 就会包含 Padding 区域的线条
        xGridPositions = ComputeGridLinePositions(extendedXMin, extendedXMax, xLabels, xLabelPriorities);
        yGridPositions = ComputeGridLinePositions(extendedYMin, extendedYMax, yLabels, yLabelPriorities);
        ...
        if (settings->showGrid) {
            ...
            for (i = 0.0; i < xGridPositions->size(); i = i + 1.0) {
                ...
                // 原代码: DrawLine1px(canvas, px, yPixelMin, px, yPixelMax, settings->gridColor);
                // 修改为: 起点 0, 终点 settings->height (全图纵向画满)
                DrawLine1px(canvas, px, 0.0, px, settings->height, settings->gridColor);
            }
            ...
            for (i = 0.0; i < yGridPositions->size(); i = i + 1.0) {
                ...
                // 原代码: DrawLine1px(canvas, xPixelMin, py, xPixelMax, py, settings->gridColor);
                // 修改为: 起点 0, 终点 settings->width (全图横向画满)
                DrawLine1px(canvas, 0.0, py, settings->width, py, settings->gridColor);
            }
        }
        ...
        /* x origin line */
        if (originYInside) {
            DrawLine1px(canvas, 0.0, Round(originYPixels), settings->width, Round(originYPixels), GetBlack());

        }

        /* y origin line */
        if (originXInside) {
            DrawLine1px(canvas, Round(originXPixels), 0.0, Round(originXPixels), settings->height, GetBlack());
            ...
            if (linearInterpolation) {
                ...
                for (i = 0.0; i < xs->size(); i = i + 1.0) {
                    ...
                    if (prevSet) {
                        ...
                        double xValuePerPixel = (xMax - xMin) / (xPixelMax - xPixelMin);
                        double yValuePerPixel = (yMax - yMin) / (yPixelMax - yPixelMin);
                        double extXMin = xMin - (xPixelMin * xValuePerPixel);
                        double extXMax = xMax + ((settings->width - xPixelMax) * xValuePerPixel);
                        double extYMin = yMin - ((settings->height - yPixelMax) * yValuePerPixel);
                        double extYMax = yMax + (yPixelMin * yValuePerPixel);

                        success = CropLineWithinBoundary(x1Ref, y1Ref, x2Ref, y2Ref, extXMin, extXMax, extYMin, extYMax);
                ...
                for (i = 0.0; i < xs->size(); i = i + 1.0) {
                    ...
                    if (x > extendedXMin && x < extendedXMax && y > extendedYMin && y < extendedYMax) {
...
void BarPlotExample(RGBABitmapImageReference* imageReference) {
    ...
    settings->showGrid = true;
    settings->gridColor = CreateRGBAColor(0.9, 0.9, 0.9, 1.0);
...
void DrawLine(RGBABitmapImage* canvas, double x1, double y1, double x2, double y2, double thickness, RGBA* color) {
    if (thickness <= 1.0) {
        ...
        double offset_limit = (thickness - 1.0) / 2.0;
        for (double i = -offset_limit; i <= offset_limit; i += 0.5) {
            if (abs(x2 - x1) < abs(y2 - y1)) {
                DrawLine1px(canvas, x1 + i, y1, x2 + i, y2, color);
            }
            else {
                DrawLine1px(canvas, x1, y1 + i, x2, y2 + i, color);
            }
        }
    }
}
